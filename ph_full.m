...
%% === PHASE 3: TRACKING + DYNAMIC SNR + KALMAN ===
...
% SNR variation: natural smooth curve
...
%% === KALMAN FILTER-BASED ANGLE TRACKING ===
num_frames = 20;
track_history = zeros(length(merged_angles), num_frames);
Q = 1; R = 2;
for i = 1:length(merged_angles)
    x = merged_angles(i); v = 1 + randn*0.2; Pk = 1;
    for t = 1:num_frames
        x = x + v; Pk = Pk + Q;
        z = x + randn()*sqrt(R);
        K = Pk / (Pk + R);
...
title('Kalman Filter-based Human Tracking');
...
title('Tracking Density Heatmap');
